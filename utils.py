...
# --- ADDED FUNCTION ---
# This new function loads the pre-computed SAM masks during training.
...
def get_train_masks_auto(paths, height=256, width=256, mode='L'):
    ...
    It's identical to get_train_images_auto but is named for clarity.
    """
    if isinstance(paths, str):
        paths = [paths]
    images = []
    for path in paths:
        # The get_image function is reused here
        image = get_image(path, height, width, mode=mode)
        if mode == 'L':
            # Masks are single-channel, so this is correct
            image = np.reshape(image, [1, image.shape[0], image.shape[1]])
        else:
            # This part is for multi-channel images, won't be used for masks
            image = np.reshape(image, [image.shape[2], image.shape[0], image.shape[1]])
        images.append(image)
